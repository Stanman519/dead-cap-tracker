...
namespace DeadCapTracker.Repositories
{
    public interface IGlobalMflApi
    {
        ...
        Task<MflByesRoot> GetByesForWeek([Path] string week, [Path] int year = Utils.ThisYear);

        [Get("{year}/export?TYPE=injuries&W={thisWeek}&JSON=1")]
        Task<InjuriesRoot> GetInjuries([Path] string thisWeek, [Path] int year = Utils.ThisYear);
    }
}
