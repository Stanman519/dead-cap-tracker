...
namespace DeadCapTracker.Repositories
{
    ...
    public interface IMflApi
    {
        ...
        Task<MflStandingsParent> GetStandings([Path] int leagueId, [Path] int year, [Path] string ApiKey);
        ...
        Task<MflAgentsParent> GetFreeAgents([Path] int leagueId, [Path] int year, [Path] string ApiKey);
        ...
        Task<TradeTransactionMulti> GetRecentTrade([Path] int leagueId, [Path] int year, [Path] string ApiKey);

        [Get("{year}/export?TYPE=tradeBait&L={leagueId}&APIKEY={ApiKey}&INCLUDE_DRAFT_PICKS=true&JSON=1")]
        Task<TradeBaitsParent> GetTradeBait([Path] int leagueId, [Path] int year, [Path] string ApiKey);
        
        [Get("{year}/export?TYPE=players&L={leagueId}&APIKEY={ApiKey}&DETAILS=&SINCE=&PLAYERS={player}&JSON=1")]
        Task<PlayerParent> GetBotPlayerDetails([Path] int leagueId, [Path] string player, [Path] int year, [Path] string ApiKey);
        
        [Get("{year}/export?TYPE=players&L={leagueId}&APIKEY={ApiKey}&DETAILS=&SINCE=&PLAYERS={players}&JSON=1")]
        Task<PlayersParent> GetBotPlayersDetails([Path] int leagueId, [Path] string players, [Path] int year, [Path] string ApiKey);
        ...
        Task<ScheduleRoot> GetMatchupSchedule([Path] int leagueId, [Path] int year, [Path] string ApiKey);

        [Get("{year}/export?TYPE=players&L={leagueId}&APIKEY={ApiKey}&DETAILS=&SINCE=&PLAYERS=&JSON=1")]
        Task<PlayersParent> GetAllMflPlayers([Path] int leagueId, [Path] int year, [Path] string ApiKey);
